<form name="form_default" id="form_default" autocomplete="off" method="POST" action="{{$url}}">
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="mb-3">
                <label for="username" class="form-label">Nomor Induk Karyawan</label>
        ...
        <div class="col-sm-6">
            <div class="mb-3">
                <label for="factory_uuid" class="form-label">Factory</label>
                <select class="form-select" name="factory_uuid" id="factory_uuid">
                    @foreach($factory_uuid as $val)
                        <option value="{{$val->uuid}}" {{ (isset($data['factory_uuid']) && $data['factory_uuid']==$val->uuid) ? 'selected' : '' }}>{{$val->nama}}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
<script>
    var username = "{{(isset($data['username'])) ? $data['username'] : ''}}";
