...
use App\Http\Controllers\ScanDokumenController;
 ...
 * gunakan perintah `php artisan generate:rolemodule`
 */
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified', 'can:Access'])->group(function () {
    ...
    // Route untuk modul scan-dokumen
    Route::group([
        'prefix' => 'scan-dokumen',
    ], function () {
        Route::get('/', [ScanDokumenController::class, 'index'])->name('scan-dokumen');

        Route::get('/create', [ScanDokumenController::class, 'create']);
    });
});
